...
const RunAudit = () => {
    ...
    try {
      ...
      if (isOnline) {
        ...
        if (auditsData.length === 0) {
          toast.warning(
            "No cached data available. Connect to internet to load audits.",
          );
        }
      }
      ...
      if (runId && isOnline) {
        ...
        if (runRes.data.completed || runRes.data.closed_at) {
          toast.error(
            runRes.data.closed_at
              ? "This audit was closed because it was not completed in time."
        ...
        const firstUnanswered =
          audit?.questions?.findIndex(
            (q) => !savedAnswers[q.id]?.response_value,
          ) ?? -1;
        setCurrentQuestionIndex(firstUnanswered >= 0 ? firstUnanswered : 0);
      } else if (!runId) {
        const requestedAuditId = new URLSearchParams(
          window.location.search,
        ).get("audit");
        const requestedAudit =
          requestedAuditId &&
          auditsData.find((item) => item.id === requestedAuditId);
        if (requestedAudit) {
          // Printable QR sheets point here. Authentication and the normal
          // audit permissions are still enforced before the run is created.
          await startAudit(requestedAudit);
        }
      }
    } catch (error) {
  ...
  const startAudit = async (audit) => {
    if (isOnline) {
      ...
      setAnswers({});
      toast.info(
        "Starting offline audit. It will sync when you're back online.",
      );
    }
  };
  ...
  const getResponseOptions = (question) => {
    if (question.response_group_id) {
      const group = responseGroups.find(
        (g) => g.id === question.response_group_id,
      );
  ...
  const handleAnswer = (question, option) => {
    ...
    setAnswers({
      ...
      [question.id]: {
        ...
        repeat_non_conformance: isNegative
          ? existingAnswer.repeat_non_conformance || false
          : false,
      },
    });
  };
  ...
  const handlePassFail = (questionId, status) => {
    ...
    setAnswers({
      ...
      [questionId]: {
        ...
        repeat_non_conformance:
          status === "fail"
            ? currentAns.repeat_non_conformance || false
            : false,
      },
    });
  };
  ...
  const handlePhotoUpload = async (e) => {
    ...
    try {
      ...
      // into its queue without returning an uploaded URL.
      if (!photoUrl) photoUrl = await fileToDataUrl(compressedFile);
      setAnswers((prev) => {
        ...
        return {
          ...
          [questionId]: {
            ...currentAnswer,
            photos: [...(currentAnswer.photos || []), photoUrl],
          },
        };
      });
      toast.success(
        isOnline ? "Photo added" : "Photo saved with this offline audit",
      );
    } catch (error) {
  ...
  const setActionOwner = (questionId, userId) => {
    const currentAnswer = answers[questionId];
    if (!currentAnswer) return;
    const selectedOwner = actionAssignees.find(
  ...
  const saveProgress = async (exitAfterSave = false) => {
    ...
    if (!isOnline || activeRun.offline) {
      toast.info(
        "Connect to the internet to save this audit as a resumable draft.",
      );
      return;
    }
    setExiting(exitAfterSave);
    try {
      const saved = await axios.put(`${API}/run-audits/${activeRun.id}`, {
      ...
      toast.success(
        exitAfterSave
          ? "Audit saved. You can continue it later."
          : "Progress saved",
      );
  ...
  const submitAudit = async () => {
    ...
    const unanswered = currentAudit.questions.filter(
      (q) => q.required && !answers[q.id]?.response_value,
    );
    if (unanswered.length > 0) {
      toast.error(
        `Please answer all required questions (${unanswered.length} remaining)`,
      );
      const firstUnanswered = currentAudit.questions.findIndex(
        (q) => q.required && !answers[q.id]?.response_value,
      );
    ...
    if (negativeWithoutComments.length > 0) {
      toast.error(
        `Comments are required for all fail/negative responses. ${negativeWithoutComments.length} missing.`,
      );
    ...
    const incompleteActions = Object.values(answers).filter(
      (a) =>
        a.is_negative &&
    ...
    if (incompleteActions.length > 0) {
      toast.error(
        `Action required, registered action owner and due date must be completed for every non-conformance. ${incompleteActions.length} incomplete.`,
      );
      const firstIncompleteIdx = currentAudit.questions.findIndex((q) =>
        incompleteActions.some((a) => a.question_id === q.id),
      );
      if (firstIncompleteIdx !== -1)
        setCurrentQuestionIndex(firstIncompleteIdx);
      return;
    }
    ...
    if (!isOnline || activeRun.offline) {
      try {
        ...
        toast.success(
          "Audit saved offline! It will sync when you're back online.",
  ...
  const progress = currentAudit
    ? (Object.keys(answers).filter((k) => answers[k]?.response_value).length /
        currentAudit.questions.length) *
      100
    : 0;
    ...
    return (
      <div className="space-y-6" data-testid="run-audit-page">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Select an audit to begin</p>
        </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  You can still run audits. They will sync automatically when
                  you&apos;re back online.
                </p>
              </div>
            </div>
        ...
        {isOnline && savedRuns.length > 0 && (
          <Card className="border-primary/30">
            <CardHeader>
              ...
              <CardDescription>
                Continue any open audit in your company. Due Friday; unfinished
                audits close at the start of Monday.
              </CardDescription>
            </CardHeader>
                ...
                >
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      Started by {run.auditor_name} •{" "}
                      {new Date(run.started_at).toLocaleString("en-GB", {
                        timeZone: "Europe/London",
                      })}{" "}
                      • Due {run.due_date?.split("-").reverse().join("/")}
                    </p>
                  </div>
                  <Button
                    onClick={() => navigate(`/run-audit/${run.id}`)}
                    data-testid={`continue-audit-${run.id}`}
                  >
              ...
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <Select
                    ...
                    onValueChange={(value) =>
                      setSelectedLineShift(value === "none" ? "" : value)
                    }
                  >
      ...
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-4 min-w-0">
          <Button
            ...
            aria-label={
              activeRun.auditor_id === user?.id || isAdmin()
                ? "Cancel audit and go back"
                : "Save audit and go back"
            }
            title="Go back"
          ...
          <div className="min-w-0">
            ...
            <p className="text-sm text-muted-foreground flex items-center gap-2 flex-wrap">
              ...
              Started by {activeRun.auditor_name} at{" "}
              {new Date(activeRun.started_at).toLocaleTimeString("en-GB", {
                timeZone: "Europe/London",
                hour: "2-digit",
                minute: "2-digit",
                hour12: false,
        ...
        >
          ...
          {exiting ? "Exiting..." : "Save & Exit"}
        </Button>
      </div>

      <p className="text-sm text-muted-foreground">
        {activeRun.offline ? (
          "This offline audit will become available to colleagues once synced."
        ) : (
          <>
            Due Friday {activeRun.due_date?.split("-").reverse().join("/")}.
            Unfinished audits automatically close at the start of Monday.
          </>
        )}
      </p>
      <Card>
        ...
        <Card className="animate-fadeIn">
            ...
            <div className="flex items-start justify-between">
              <div>
                <div className="flex items-center gap-2 mb-2">
                  <Badge
                    variant={currentQuestion.required ? "default" : "secondary"}
                  >
          ...
          <CardContent className="space-y-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                {options.map((option, i) => (
                  <Button
                    ...
                    className={`h-auto py-4 px-3 flex flex-col items-center gap-1 ${currentAnswer?.response_value === option.value ? (option.label.toLowerCase().includes("pass") || option.label.toLowerCase().includes("yes") || option.label.toLowerCase().includes("accept") ? "bg-emerald-600 hover:bg-emerald-700 border-emerald-600" : option.label.toLowerCase().includes("fail") || option.label.toLowerCase().includes("no") || option.label.toLowerCase().includes("reject") ? "bg-red-600 hover:bg-red-700 border-red-600" : "") : ""}`}
            ...
            {questionType === "text" && (
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    handleTextAnswer(currentQuestion, e.target.value)
                  }
                  rows={4}
            ...
            {currentAnswer?.is_negative && (
              <div
                className="flex items-center justify-between rounded-lg border border-amber-300 bg-amber-50/70 p-3 dark:border-amber-900 dark:bg-amber-950/20"
                data-testid="repeat-nc-toggle"
              >
                <div>
                  <Label htmlFor={`repeat-nc-${currentQuestion.id}`}>
                    Repeat Non-Conformance
                  </Label>
                  <p className="text-xs text-muted-foreground">
                    Counts this failure as 2 non-conformances.
                  </p>
                </div>
                <input
                  id={`repeat-nc-${currentQuestion.id}`}
                  type="checkbox"
                  className="h-5 w-5 accent-red-600"
                  checked={!!currentAnswer.repeat_non_conformance}
                  onChange={(e) =>
                    updateActionField(
                      currentQuestion.id,
                      "repeat_non_conformance",
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                >
                  {currentAnswer?.is_negative
                    ? "Comment (Required for negative response)"
                    : "Notes (Optional)"}
                  {currentAnswer?.is_negative &&
              ...
              <Textarea
                ...
                onChange={(e) =>
                  addNoteToAnswer(currentQuestion.id, e.target.value)
                }
            ...
            {currentAnswer?.is_negative && (
              <div
                className="space-y-4 rounded-lg border border-red-200 bg-red-50/60 p-4 dark:border-red-900 dark:bg-red-950/20"
                data-testid="corrective-action-fields"
              >
                <div>
                  <div className="flex items-center gap-2">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Create the action and give it one accountable registered
                    owner.
                  </p>
                </div>

                <div className="space-y-2">
                  <Label htmlFor={`action-required-${currentQuestion.id}`}>
                    Action Required *
                  </Label>
                  <Textarea
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value) =>
                      setActionOwner(currentQuestion.id, value)
                    }
                  >
                    <SelectTrigger data-testid="action-assigned-user">
                  ...
                  {actionAssignees.length > 0 ? (
                    <p className="text-xs text-muted-foreground flex items-start gap-1.5">
                      ...
                      The owner will receive the assignment email and will be
                      responsible for reviewing and signing off the action after
                      completion.
                    </p>
                  ) : (
                    <p className="text-xs text-destructive">
                      No registered users are available from the cached/company
                      user list. Connect to the internet or ask an administrator
                      to add the required user before submitting this
                      non-conformance.
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-2 sm:max-w-xs">
                  <Label htmlFor={`action-due-${currentQuestion.id}`}>
                    Due Date *
                  </Label>
                  <Input
          ...
          <Button
            onClick={() =>
              setCurrentQuestionIndex((prev) =>
                Math.min((currentAudit?.questions.length || 1) - 1, prev + 1),
              )
            }
            ...
            {currentAudit?.questions.map((q, i) => {
              ...
              return (
                <button
                  ...
                  className={`w-8 h-8 rounded-md text-sm font-medium transition-colors ${failed ? "bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400" : i === currentQuestionIndex ? "bg-primary text-primary-foreground" : answered ? "bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400" : "bg-muted text-muted-foreground hover:bg-muted/80"}`}
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm font-medium" data-testid="signoff-name">
                {user?.name}
              </p>
            </div>
            <div>
              <Label className="text-xs text-muted-foreground">Email</Label>
              <p className="text-sm font-medium" data-testid="signoff-email">
                {user?.email}
              </p>
            </div>
          </div>
