...
const DocumentView = () => {
  ...
  const formatDate = (iso) => {
    if (!iso) return '-';
    return new Date(iso).toLocaleString('en-GB', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', timeZone: 'Europe/London' });
  };
  ...
  return (
    <div className="space-y-6" data-testid="document-view-page">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mt-1 flex-wrap">
              ...
              <Badge className={doc.completed ? 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-amber-100 text-amber-800'}>{doc.completed ? 'Completed' : 'In Progress'}</Badge>
        ...
        {doc.completed && (
          <div className="flex gap-2 flex-wrap">
            <Button variant="outline" onClick={downloadPdf} disabled={downloading} data-testid="download-pdf-btn">
              {downloading ? <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin mr-2" /> : <Download className="w-4 h-4 mr-2" />}
              Download PDF
            </Button>
            <Button variant="outline" onClick={() => setEmailOpen(true)} data-testid="email-pdf-btn"><Mail className="w-4 h-4 mr-2" />Email PDF</Button>
          </div>
        )}
      </div>
      ...
      {headerFields.length > 0 && (
        <Card><CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><LayoutList className="w-4 h-4 text-primary" />Document Details</CardTitle></CardHeader><CardContent><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{headerFields.map((field, idx) => { const fv = (doc.field_values || []).find(v => v.field_id === field.id); return <div key={field.id} className="py-2 border-b last:border-0" data-testid={`view-header-${idx}`}><p className="text-xs text-muted-foreground">{field.label}</p><p className="font-medium">{getDisplayValue(field, fv?.value)}</p></div>; })}</div></CardContent></Card>
      )}

      {tableFields.length > 0 && (doc.table_rows || []).length > 0 && (
        <Card><CardHeader className="pb-3"><CardTitle className="text-base flex items-center gap-2"><Table2 className="w-4 h-4 text-primary" />Production Data ({doc.table_rows.length} rows)</CardTitle></CardHeader><CardContent><div className="overflow-x-auto border rounded-lg"><table className="w-full text-sm"><thead><tr className="bg-muted/50 border-b"><th className="px-3 py-2 text-left font-medium text-muted-foreground">#</th>{tableFields.map(f => <th key={f.id} className="px-3 py-2 text-left font-medium text-muted-foreground">{f.label}</th>)}</tr></thead><tbody>{doc.table_rows.map((row, ri) => <tr key={ri} className="border-b last:border-0 hover:bg-muted/30" data-testid={`view-row-${ri}`}><td className="px-3 py-2 text-muted-foreground font-medium">{ri + 1}</td>{tableFields.map(f => <td key={f.id} className="px-3 py-2">{getDisplayValue(f, row[f.id])}</td>)}</tr>)}</tbody></table></div></CardContent></Card>
      )}
      ...
      <EmailReportDialog
        ...
        endpoint={`/reports/documents/${documentId}/email`}
        title="Email document"
        description="Send this completed document PDF as an email attachment."
      />
    </div>
  );
};
