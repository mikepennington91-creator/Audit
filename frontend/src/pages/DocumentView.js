...
import { ArrowLeft, Download, FileText, Calendar, User, Hash, Table2, LayoutList } from 'lucide-react';
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {headerFields.map((field, idx) => {
                ...
                return (
                  <div key={field.id} className="py-2 border-b last:border-0" data-testid={`view-header-${idx}`}>
                    <p className="text-xs text-muted-foreground">{field.label}</p>
                    <p className="font-medium">{getDisplayValue(field, fv?.value)}</p>
                  </div>
                );
              })}
            </div>
        ...
        <Card>
          <CardHeader className="pb-3">
            <CardTitle className="text-base flex items-center gap-2">
              <Table2 className="w-4 h-4 text-primary" />Production Data ({doc.table_rows.length} rows)
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-muted/50 border-b">
                    ...
                    {tableFields.map(f => (
                      <th key={f.id} className="px-3 py-2 text-left font-medium text-muted-foreground">{f.label}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {doc.table_rows.map((row, ri) => (
                    <tr key={ri} className="border-b last:border-0 hover:bg-muted/30" data-testid={`view-row-${ri}`}>
                      <td className="px-3 py-2 text-muted-foreground font-medium">{ri + 1}</td>
                      {tableFields.map(f => (
                        <td key={f.id} className="px-3 py-2">
                          {getDisplayValue(f, row[f.id])}
                        </td>
                      ))}
                    </tr>
                  ))}
